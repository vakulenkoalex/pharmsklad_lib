...
Функция ВызватьМетод(Знач HTTPМетод, Знач HTTPЗапрос)
	...
	Логгер.Отладка("HTTP.Запрос", УникальныйКодЗапроса, СериализацияЗапроса);
	
	Попытка
		HTTPОтвет = HTTPСоединение.ВызватьHTTPМетод(HTTPМетод, HTTPЗапрос);
	Исключение
		Логгер.Ошибка("HTTP.Запрос", УникальныйКодЗапроса, СериализацияЗапроса);
		Логгер.Ошибка("HTTP.ВызватьМетод", УникальныйКодЗапроса, Новый Структура("ПодробноеПредставлениеОшибки", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		ВызватьИсключение;
	КонецПопытки;
	
	Логгер.Отладка("HTTP.Ответ", УникальныйКодЗапроса, СериализацияОтвета(HTTPОтвет));
