...
Процедура Инициализация(Знач л_Логгер, Знач ПараметрыСоединение) Экспорт
...
Функция ВызватьМетод(Знач HTTPМетод, Знач HTTPЗапрос)
	...
	Исключение
		...
		Логгер.Ошибка("ВызватьHTTPМетод", УникальныйКодЗапроса, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
