...
Перем ИерархияСобытия;
Перем ОбъектМетаданных;
Перем ИмяКлиента;
Перем ПолноеИмяФайла;
Перем ИмяУзла;
Перем ОтладкаВключена;
Перем ЗаписьВФайлВключена;
Перем СообщенияПользователюВключены;
...
Процедура ЗаписатьВЛог(Знач ИмяСобытия, Знач Сообщение, Знач Уровень, Знач Данные = Неопределено)
	...
	ПредставлениеСообщения = ПолучитьПредставлениеСообщения(Сообщение);
...
Функция ПолучитьПредставлениеСообщения(Знач Сообщение)
...
Функция ПолучитьИмяУзла()
	Возврат фмЛоггерПереопределяемый.ПолучитьИмяУзла();
КонецФункции
