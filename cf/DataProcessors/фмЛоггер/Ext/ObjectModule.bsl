...
Функция ДанныеВJson(Знач Данные)
	...
	Если Данные <> Неопределено Тогда
		...
		Попытка
			...
			ДанныеДляСериализации = Данные;
		Исключение
			Ошибка("ОшибкаВТипеДанныхДляЛоггера", "нужно передавать данные в логгер в виде структуры");
			ДанныеДляСериализации = Новый Структура("ОшибкаВТипеДанныхДляЛоггера", Данные);
		КонецПопытки;
		
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(КопияДанных, ДанныеДляСериализации);
		
	КонецЕсли;
	
	Если КопияДанных.Количество() > 0 Тогда
		Результат = Сериализация(КопияДанных, "");
	КонецЕсли;
