...
// Массив значений колонки таблицы.
//
// Параметры:
//  МенеджерВременныхТаблиц	 - МенеджерВременныхТаблиц - менеджер содержащий таблицу.
...
//
Функция МассивЗначенийКолонкиТаблицы(Знач МенеджерВременныхТаблиц, Знач ИмяТаблицы, Знач ИмяКолонки) Экспорт
	
	ВременнаяТаблицаЗапроса = МенеджерВременныхТаблиц.Таблицы.Найти(ИмяТаблицы);
	Если ВременнаяТаблицаЗапроса = Неопределено Тогда
		ВызватьИсключение СтрШаблон("Таблица с именем ""%1"" не найдена в МенеджерВременныхТаблиц", ИмяТаблицы);
	КонецЕсли;
	Если ВременнаяТаблицаЗапроса.Колонки.Найти(ИмяКолонки) = Неопределено Тогда
		ВызватьИсключение СтрШаблон("Колонка с именем ""%1"" не найдена в таблице ""%2""", ИмяКолонки, ИмяТаблицы);
	КонецЕсли;
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ИмяКолонки", ИмяКолонки);
